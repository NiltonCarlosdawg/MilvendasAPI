...
const Events = () => {
  ...
  if (eventItems.length === 0) {
    return (
      <section className="min-h-screen py-24 bg-white dark:bg-slate-900">
            ...
            >
              <div className="flex-1">
                ...
                <div className="space-y-3 mb-8">
                  <div className="flex items-center gap-3 text-sm font-medium text-slate-500"><Clock size={16} /> {new Date(event.eventDate).toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' })}</div>
                  <div className="flex items-center gap-3 text-sm font-medium text-slate-500"><MapPin size={16} /> {event.location}</div>
                  <div className="flex items-center gap-3 text-sm font-medium text-slate-500"><Users size={16} /> Cap: {event.capacity || 'Ilimitada'}</div>
                </div>
              </div>
              ...
              {showSubscriptionForm === event.id ? (
                <form onSubmit={handleSubmit} className="space-y-3 bg-white dark:bg-slate-900 p-4 rounded-3xl border border-blue-100 dark:border-blue-900">
                  <input required placeholder="Nome" className="w-full p-3 text-sm bg-slate-50 dark:bg-slate-800 rounded-xl" value={subscriptionData.name} onChange={e => setSubscriptionData({...subscriptionData, name: e.target.value})} />
                  <input required type="email" placeholder="Email" className="w-full p-3 text-sm bg-slate-50 dark:bg-slate-800 rounded-xl" value={subscriptionData.email} onChange={e => setSubscriptionData({...subscriptionData, email: e.target.value})} />
                  <div className="flex gap-2 pt-2">
                    <button disabled={submitting} type="submit" className="flex-1 py-3 bg-blue-600 text-white text-sm font-bold rounded-xl">{submitting ? '...' : 'Confirmar'}</button>
                    <button type="button" onClick={() => setShowSubscriptionForm(null)} className="px-4 py-3 bg-slate-100 dark:bg-slate-800 text-sm font-bold rounded-xl">X</button>
                  </div>
                </form>
              ) : (
                <div className="flex gap-3">
                  <button onClick={() => handleSubscribeClick(event.id, event.title)} className="flex-1 py-4 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20">Quero Participar</button>
                  <a href={`https://wa.me/244922965959?text=Informações: ${event.title}`} className="p-4 bg-green-500 text-white rounded-2xl hover:bg-green-600 transition-all"><Phone size={20} /></a>
                </div>
              )}
            </motion.div>
          ))}
